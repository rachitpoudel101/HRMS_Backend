...
class BranchSerializer(DynamicFieldsModelSerializer, BaseAuditSerializer):
    ...
    def validate_code(self, value):
        if Branch.objects.filter(code=value).exclude(id=self.instance.id if self.instance else None).exists():
            raise serializers.ValidationError("Branch code must be unique within the company.")
        return value
    
    def validate_name(self, value):
        if Branch.objects.filter(name=value).exclude(id=self.instance.id if self.instance else None).exists():
