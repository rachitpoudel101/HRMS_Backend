...
class NoticSerializer(serializers.ModelSerializer):
    ...
    employees_details = EmployeeSerializer(
        source="employees", many=True, read_only=True
    )
    attachments_details = NoticeAttachmentListSerializer(
        source="attachments", many=True, read_only=True
    )
...
class NoticCreateUpdateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Notic
        fields = ["id", "name", "date", "description", "branch", "employees"]
        read_only_fields = ["id"]
